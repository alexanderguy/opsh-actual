...
struct param_exp {
    ...
    word_part_t *index; /* array index expression, NULL for non-array */
    bool index_all;     /* true for [@] or [*] */
};
...
struct command {
    ...
    union {
        struct {
            ...
            plist_t array_names;    /* plist of char* (array assignment names) */
            plist_t array_elements; /* plist of plist_t* (array element word lists) */
        } simple;                   /* CT_SIMPLE */
        struct {
            sh_list_t *body;
